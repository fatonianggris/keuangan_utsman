...
if (sha1($_JSON['nomorPembayaran'] . $secret_key . $_JSON['tanggalTransaksi'] . $totalNominal . $nomorJurnalPembukuan) != $_JSON['checksum']) {
    $response = json_encode(array(
        'rc' => 'ERR-SECURE-HASH',
...
try {
    $sql = $conn->prepare("UPDATE tagihan_pembayaran set status_pembayaran=?, nomor_jurnal_pembukuan=?, waktu_transaksi=?, channel_pembayaran=? WHERE id_invoice = ?");
    $status = "SUKSES";
    $waktu_pembayaran = date("Y-m-d H:i:s");
    debugLog("XXX");
    $sql->bind_param('sssss', $status, $nomorJurnalPembukuan, $waktu_pembayaran, $kodeChannel, $id_tagihan);
    debugLog("YYY");
    $sql->execute();
    $conn->commit();
} catch (Exception $e) {
